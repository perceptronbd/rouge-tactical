...
const updateService = async (req, res) => {
  const { serviceId, service, accountNumber, cost, dueDate } = req.body;
...
const fetchServiceCost = async (req, res) => {
  try {
    ...
    const filteredServices = allServices.filter(
      (service) => service.dueDate >= intervalEndDate && service.dueDate <= now
    );
...
const filterInvoice = async (req, res) => {
  try {
    ...
    const filteredInvoice = allInvoice.filter(
      (invoice) =>
    ...
    filteredInvoice.forEach((invoice) => {
      ...
      totalsByMonth[key].dueAmount +=
        parseFloat(invoice.totalAmount) - parseFloat(invoice.depositAmount);
    });
