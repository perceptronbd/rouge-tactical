  ...
  try {
    ...
    if (!machine || !condition || !maintenanceInterval || !status) {
      return res.status(400).json({
        message:
          "All required fields must be provided for creating Maintenance",
      });
    }
...
const deleteMaintenance = async (req, res) => {
  try {
    ...
    const deletedMaintenance = await Maintenance.findByIdAndRemove(
      maintenanceId
    );
  ...
  try {
    ...
    if (status === "complete") {
      ...
      const nextMaintenanceDate = new Date(
        currentDate.getTime() + maintenanceInterval * 24 * 60 * 60 * 1000
      );
